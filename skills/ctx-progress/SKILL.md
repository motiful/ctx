...
**Two gates, two granularities — the tracking gate must not govern the SOT.** Sink-same-turn is the **tracking** gate. It keeps `progress/` current-to-the-turn and deliberately **lean**: the current position, the next step, and *pointers* to decided state — not the blow-by-blow. Options weighed, reasoning, and detail live in `reports/` and git; they never bloat progress. It is loose-but-current: track the *shape*, not every detail (tracking tightly ≠ writing verbosely). It does **NOT** license sinking to the **SOT** (`spec/` + `decisions/`), which has a far stricter gate: a product truth or an ADR sinks **only when the choice is rigorously certain and owner-decided** (ctx-spec's *record-only-decided-choices* + significance bar) — not every turn, and **never** a design the agent merely synthesized this turn that the owner has not approved. Progress may record *"we are weighing X"* now; the conclusion *X* reaches `spec/`/`decisions/` only once X is actually decided. Letting the same-turn tracking cadence drag a half-settled design into the SOT is the exact failure this separation prevents.
