...
4. **What + how first, why later.** Open with what's proposed and how it works; put rationale in later sections.
   ...
   **When the report covers a process — a run, a batch, a migration, an audit — the "what" is what the DELIVERABLE can now do, never how the process went.** These are easy to confuse because the process is what you just spent your effort on, so it is what is loaded and vivid; the reader spent nothing on it and is asking a different question. A report that opens with agent counts, losses found, checks that failed, and rules earned has answered *"was the machinery sound?"* — a question only its author was asking. Lead instead with the capabilities the artifact gained, each stated as something it could not do before and can do now, and put the machinery in a later section or drop it. **The tell that you got this wrong is a reader saying some version of *"I read all that and I still don't know what improved"* — and note that they will say it about a report that is accurate, well-organised, and quantified, because none of those properties fix the wrong subject.** The same rule sets the *order*: within a batch report, a capability the work unlocked outranks a defect the work caught, even when the defect took ten times the effort.

   **The capability framing is right for work that ADDS and wrong for work that ADJUDICATES, and reaching for it out of habit destroys the one thing an audit reader needs.** An audit's items do not share a status: some are things that were missing and are now in, some are things that are present and correct but whose grounds have collapsed, some are things checked and cleared. *"What can it now do"* flattens all three into one voice, and the reader cannot tell which he is looking at — **not whether it matters, but whether it is a fix, a warning, or a non-event.** One report put nine recovered omissions and nine still-standing-but-undermined entries under a single heading meaning roughly *recovered*, and the owner's reply was **"I can't even tell what your position is"** — not a complaint about depth, about *stance*. **So: when the items in a report do not share a status, status is a column, not a sentence.** State the classes and their counts before any individual item, give each item its own status marker, and say for each what was done to it — *added here / flagged in place / checked and cleared*. **The test: can the reader answer "is this thing right or wrong, and is it fixed or not" without reading a paragraph?** If the answer only emerges from prose, the classification is doing no work.

   **And the *scope* is this round only.** A report on a run covers what that run did, what it produced, and what it needs decided — not the background of the project, not what earlier rounds produced, not a glossary of terms the reader coined. The pull toward recap is strongest right after a context reset, because the history is what the *author* just reloaded and so it feels like shared ground being established; the reader never lost it. **A report that opens by explaining what the project is has cast its own commissioner as someone who needs onboarding.** The one thing from an earlier round that belongs here is something *this* round overturned or corrected — that is current work, not history. Everything else fails a one-line check: **does the reader already know this? Then cut it.** Corollary: the decisions being asked for are the report's *purpose*, not its appendix — put them where they cannot be missed.
...
6. **End by asking for a verdict.** Close with an explicit "for you to decide" list + options, so the reader gives a clear judgment instead of hunting for the open points. **Gate what lands in that list by confidence × blast-radius:** high-confidence + low-blast findings you can just state as done/recommended; anything high-blast OR low-confidence is what you escalate here as an explicit decision for the human. **But run the lookup before the gate: an unanswered question and an un-looked-up question are indistinguishable from the inside, and escalating one feels like diligence.** On one report every single item in the verdict list — four of four — was settled by a search its author had not run: one by a rule in the governing procedure, one by the project's own schedule, one by a single shell command, one by a precedent an earlier round had already ruled and written down. **So for each candidate item, confirm none of these four answers it: (a) a rule you are already operating under, (b) the plan or schedule, (c) a precedent, (d) a command you could run right now.** Only what survives all four belongs in front of a human; asking about the rest spends their attention on your unfinished work. **Record each of the four lookups' results next to the item — an item with no recorded lookup is not ready to escalate.** Held as a mental checklist this rule reliably fails to fire: it was already written here, in these words, after a first occurrence, and a later report still escalated seven items of which five were already settled — one by a ruling the owner had given the day before and that was recorded in two files the author had read that same session. A check you read is not a check you ran; only its output proves it ran. Don't bury a colossal-or-uncertain call in prose, and don't pad the verdict list with reversible things you were sure of.

   **Each verdict item MUST be decidable from itself.** Naming the item is not asking the question. A row reading "the 10 dropped rows — review?" tells the reader there is something to look at and nothing about whether to look; deciding it still requires opening a ledger, learning what a disposition is, and reconstructing why those ten differ from the other 970. **So every escalated item carries, inline: what it is in plain words, what was decided and by whom, what changed since, the two things being chosen between, and what each choice costs.** A verdict list is where rule 1 and rule 3 matter most and where they are abandoned most — a table is the natural shape for a list of items and the worst shape for a decision, because its cells are too narrow to hold a reason. If an item genuinely needs more room than a row, give it a section and leave the table for the summary.
